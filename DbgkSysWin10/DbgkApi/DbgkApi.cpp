    ...
    IN ULONG Flags)
{
    ...
    /* Return Status */
    DBGKTRACE(DBGK_OBJECT_DEBUG, "句柄：%p；调试对象：%p\n",
        hDebug, DebugObject);
    return Status;
}
    ...
    IN PDEBUG_OBJECT TargetObject OPTIONAL)
{
    ...
    else
    {
        ...
        if (!(DebugObject->Flags & DEBUG_OBJECT_DELETE_PENDING))
        {
            ...
            DBGKTRACE(DBGK_MESSAGE_DEBUG, "正在插入调试事件：%p；API 编号：%d\n",
    ...
    /* Return status */
    DBGKTRACE(DBGK_MESSAGE_DEBUG, "状态：%lx\n", Status);
    return Status;
}
    ...
    IN BOOLEAN SecondChance)
{
    ...
    if (IsUseDebugPort)  //是否将异常转发到调试端口
    {
        ...
        {
            Port = DebugProcess->DebugObject;
            outLog("Dbgk 调试对象句柄：%llX", DebugProcess->DebugObject);
        }
        else
        {
            size_t ptr_DebugPort = (size_t)Process + eprocess_offset::DebugPort;
...
--*/
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        DeleteDebugProcess(DebugObject);
        DbgPrint("清理 Dbgk 调试对象完成；NTSTATUS=0x%x；如失败，请检查对象状态和符号表。", Status);
        ObDereferenceObject(DebugObject);
    }
    ...
    IN _ETHREAD* LastThread)
{
    ...
    PAGED_CODE();
    DBGKTRACE(DBGK_PROCESS_DEBUG, "进程：%p；调试对象：%p\n",
        Process, DebugObject);
    ...
    while (NextEntry != &DebugObject->EventList)
    {
        ...
        NextEntry = NextEntry->Flink;
        DBGKTRACE(DBGK_PROCESS_DEBUG, "调试事件：%p；标志：%lx；线程：%p/%p\n",
            ...
            (DebugEvent->BackoutThread == CurrentThread))
        {
            ...
            DBGKTRACE(DBGK_PROCESS_DEBUG, "事件线程：%p；消息状态：%lx\n",
    ...
    IN HANDLE DebugHandle)
{
    ...
    PAGED_CODE();
    DBGKTRACE(DBGK_PROCESS_DEBUG, "进程：%p；句柄：%p\n",
    ...
    OUT PDBGUI_WAIT_STATE_CHANGE StateChange)
{
    ...
    PAGED_CODE();
    DBGKTRACE(DBGK_OBJECT_DEBUG, "句柄：%p\n", DebugHandle);
        ...
        else
        {
            ...
            while (ListHead != NextEntry)
            {
                ...
                DebugEvent = CONTAINING_RECORD(NextEntry,
                    DEBUG_EVENT,
                    EventList);
                DBGKTRACE(DBGK_PROCESS_DEBUG, "调试事件：%p；标志：%lx\n",
                    DebugEvent, DebugEvent->Flags);
