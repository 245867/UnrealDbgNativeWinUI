...
static const char* IoctlName(ULONG code)
{
    ...
    case IOCTL_LOAD_SYMBOLS_TABLE_V2: return "加载符号表";
    case IOCTL_LOAD_DEBUGGER_STATE:
    case IOCTL_LOAD_DEBUGGER_STATE_V2: return "加载调试器状态";
    case IOCTL_LOAD_PROTECT_OBJ_DATA:
    case IOCTL_LOAD_PROTECT_OBJ_DATA_V2: return "加载保护对象数据";
    case IOCTL_LOAD_DEBUGGER_DATA:
    case IOCTL_LOAD_DEBUGGER_DATA_V2: return "加载调试器数据";
    case IOCTL_CREATE_REMOTE_THREAD:
    case IOCTL_CREATE_REMOTE_THREAD_V2: return "创建远程线程";
    case IOCTL_GET_PROCESS_INFO:
    case IOCTL_GET_PROCESS_INFO_V2: return "获取进程信息";
    case IOCTL_TL_BLOCK_RESUME_THREAD:
    case IOCTL_TL_BLOCK_RESUME_THREAD_V2: return "TL 阻止恢复线程";
    case IOCTL_SET_HARDWARE_BREAKPOINT:
    case IOCTL_SET_HARDWARE_BREAKPOINT_V2: return "设置硬件断点";
    case IOCTL_DEL_HARDWARE_BREAKPOINT:
    case IOCTL_DEL_HARDWARE_BREAKPOINT_V2: return "删除硬件断点";
    case IOCTL_SET_SOFTWARE_BREAKPOINT:
    case IOCTL_SET_SOFTWARE_BREAKPOINT_V2: return "设置软件断点";
    case IOCTL_DEL_SOFTWARE_BREAKPOINT:
    case IOCTL_DEL_SOFTWARE_BREAKPOINT_V2: return "删除软件断点";
    case IOCTL_READ_SOFTWARE_BREAKPOINT:
    case IOCTL_READ_SOFTWARE_BREAKPOINT_V2: return "读取软件断点";
    default: return "未知 IOCTL";
    }
}
...
static const char* NtStatusExplanation(NTSTATUS status)
{
    switch (status)
    {
    case STATUS_SUCCESS: return "操作成功";
    case STATUS_INVALID_PARAMETER: return "参数无效；检查 IOCTL 输入结构和长度";
    case STATUS_ACCESS_DENIED: return "访问被拒绝；检查管理员权限、驱动状态和安全策略";
    case STATUS_NOT_FOUND: return "对象未找到；检查符号表、设备链接和目标进程";
    case STATUS_OBJECT_NAME_NOT_FOUND: return "对象名称未找到；检查驱动设备和符号模块是否已创建";
    case STATUS_BUFFER_TOO_SMALL: return "缓冲区不足；按要求增大输入或输出缓冲区";
    case STATUS_INVALID_DEVICE_REQUEST: return "不支持的 IOCTL；确认前端与驱动版本匹配";
    case STATUS_NOT_SUPPORTED: return "当前硬件或虚拟化环境不支持该操作；检查 Hyper-V/VBS/HVCI 冲突";
    case STATUS_DEVICE_NOT_READY: return "VT host 尚未就绪；确认 VT_Driver 已进入 VMX root mode";
    case STATUS_INSUFFICIENT_RESOURCES: return "内核资源不足；释放资源并检查非分页内存";
    case STATUS_ACCESS_VIOLATION: return "访问用户缓冲区时发生异常；检查指针、长度和调用方版本";
    case STATUS_UNSUCCESSFUL: return "驱动操作未成功；查看此前的错误日志确定具体原因";
    default: return "未收录的 NTSTATUS；请结合状态码和前置日志排查";
    }
}
EXTERN_C
VOID Unload(PDRIVER_OBJECT DriverObject)
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[DbgkSysWin10] 开始卸载\n"));
    //UnloadProtect();
    RemoveHooks();
    //DbgkUnInitialize();
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[DbgkSysWin10] 卸载完成\n"));
}
...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, NT_SUCCESS(logStatus) ? DPFLTR_INFO_LEVEL : DPFLTR_ERROR_LEVEL,
        "[DbgkSysWin10] 日志初始化状态=0x%08X\n", logStatus));

    //DbgBreakPoint();
    //ApcCreateRemoteThread((HANDLE)7572, NULL);
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[DbgkSysWin10] 驱动入口开始\n"));
    ...
    {
        outLog("创建设备对象失败；原因：目标符号地址无效、状态不匹配或资源不足；解决方案：检查匹配版本的符号表、驱动状态和内存池后重试。");
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "[DbgkSysWin10] 创建设备对象失败，状态=0x%08X\n", ntStatus));
        return ntStatus;
    }
    else
    {
        outLog("创建设备对象成功。");
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[DbgkSysWin10] 设备和符号链接创建成功\n"));
    }
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[DbgkSysWin10] 驱动入口完成\n"));
...
VOID SetupHook_DbgkCreateThread_CMP_Debugport()
{
    ...
    for (;;)
    {
            ...
            (*(WORD*)&hookAddr[3] == eprocess_offset::DebugPort))
        {
            ...
            {
                outLog("挂钩 DbgkCreateThread_CMP_Debugport_01成功。");
            }
            else
            {
                outLog("挂钩 DbgkCreateThread_CMP_Debugport_01失败；原因：目标符号地址无效、状态不匹配或资源不足；解决方案：检查匹配版本的符号表、驱动状态和内存池后重试。");
            }
            break;
        }
        else
...
VOID SetupHook_PspExitThread_CMP_Debugport()
{
        ...
        (*(WORD*)&hookAddr[3] == eprocess_offset::DebugPort))  /*if ( Process->DebugPort )*/
    {
        ...
        {
            outLog("挂钩 PspExitThread_CMP_Debugport_01成功。");
        }
        else
        {
            outLog("挂钩 PspExitThread_CMP_Debugport_01失败；原因：目标符号地址无效、状态不匹配或资源不足；解决方案：检查匹配版本的符号表、驱动状态和内存池后重试。");
        }
    }
}
...
NTSTATUS CreateDevice(IN PDRIVER_OBJECT pDriver_Object)
{
    ...
    // 将设备访问限制为 SYSTEM 和内置 Administrators。旧 V1 IOCTL 仍
    // 可供已部署组件使用，但设备 DACL 是第一道访问控制边界。
...
NTSTATUS InitDispatchRoutin(IN PDEVICE_OBJECT pDevObj, IN PIRP pIrp)
{
    ...
    // CREATE/CLOSE never return a payload.  Do not expose a union member from
    // the IRP stack as a fictitious byte count.
...
NTSTATUS HandlerDispatchRoutin(IN PDEVICE_OBJECT pDevObj, IN PIRP pIrp)
{
    ...
    if (pIrp == NULL)
    {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "[DbgkSysWin10] IOCTL 分发收到空 IRP\n"));
    ...
    if (stack == NULL)
    {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "[DbgkSysWin10] 获取当前 IRP 栈失败：返回空指针\n"));
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "[DbgkSysWin10] IOCTL 开始，代码=0x%08X 名称=%s 输入=%lu 输出=%lu 进程 ID=%lu\n",
    ...
    if (pIrp->AssociatedIrp.SystemBuffer == NULL || cbin != sizeof(USER_DATA))
    {
        ...
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "[DbgkSysWin10] IOCTL 拒绝：缓冲输入无效，地址=%p 大小=%lu，期望至少=%zu；原因：调用方未提供完整的 METHOD_BUFFERED 输入；解决方案：检查 IOCTL 结构体版本和输入缓冲区长度。\n",
        ...
        (requestData->uSize != 0 && requestData->pUserData == 0))
    {
        ntStatus = STATUS_INVALID_PARAMETER;
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "[DbgkSysWin10] IOCTL 审计拒绝：负载长度=%lu、用户指针=%p；允许上限=%lu。原因：长度越界或非零负载没有指针；解决方案：使用匹配版本 DLL，按 USER_DATA 提交有效加密缓冲区。\n",
    ...
    case IOCTL_LOAD_SYMBOLS_TABLE_V2:
    {
        ...
        if (cbout < sizeof(DWORD))
        {
            ...
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
                "[DbgkSysWin10] 符号表 IOCTL 拒绝：输出缓冲区=%lu，小于 DWORD；解决方案：调用方提供至少 4 字节输出缓冲区。\n", cbout));
            break;
        }
...
complete:
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, NT_SUCCESS(ntStatus) ? DPFLTR_INFO_LEVEL : DPFLTR_ERROR_LEVEL,
        "[DbgkSysWin10] IOCTL 完成，代码=0x%08X 名称=%s 状态=0x%08X（%s） 实际返回字节数=%lu\n",
...
VOID InitFileList(PRING3_PROTECT_OBJECT pProtectObj)
{
    ...
    __except (EXCEPTION_EXECUTE_HANDLER)
    {
        outLog("初始化文件列表崩溃；请检查保护列表数据格式和内核内存池。");
    }
}
...
VOID InitWindowList(PRING3_PROTECT_OBJECT pProtectObj)
{
    ...
    __except (EXCEPTION_EXECUTE_HANDLER)
    {
        outLog("初始化窗口列表崩溃；请检查窗口列表数据格式和内核内存池。");
    }
}
...
//        else
//        {
//            outLog("PsLookupProcessByProcessId 失败；原因：目标符号地址无效、状态不匹配或资源不足；解决方案：检查匹配版本的符号表、驱动状态和内存池后重试。");
//        }
    }
    ...
    __try
    {
        //if (IsDebugger(NULL))
        {
            /*        if (*(ULONG64*)PspLoaderInitRoutine == (ULONG64)ApcFunc)
                    {
                        outLog("APC 函数：%p；用户函数：%p；RCX：%p", ApcFunc, pUserfunc, ((PCONTEXT)pUserfunc)->Rcx);
                    }  */
