...
VOID Unload(PDRIVER_OBJECT DriverObject)
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[DbgkSysWin11] 开始卸载\n"));
    //UnloadProtect();
    RemoveHooks();
    //DbgkUnInitialize();
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[DbgkSysWin11] 卸载完成\n"));
}
...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
{
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, NT_SUCCESS(logStatus) ? DPFLTR_INFO_LEVEL : DPFLTR_ERROR_LEVEL,
        "[DbgkSysWin11] 日志初始化状态=0x%08X\n", logStatus));

    //DbgBreakPoint();
    //ApcCreateRemoteThread((HANDLE)7572, NULL);
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[DbgkSysWin11] 驱动入口开始\n"));
    ...
    if (!NT_SUCCESS(ntStatus))
    {
        ...
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "[DbgkSysWin11] 创建设备对象失败，状态=0x%08X\n", ntStatus));
        return ntStatus;
    }
    else
    {
        outLog("创建设备对象成功。");
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[DbgkSysWin11] 设备和符号链接创建成功\n"));
    }
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "[DbgkSysWin11] 驱动入口完成\n"));
...
NTSTATUS HandlerDispatchRoutin(IN PDEVICE_OBJECT pDevObj, IN PIRP pIrp)
{
    ...
    if (pIrp == NULL)
    {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "[DbgkSysWin11] IOCTL 分发收到空 IRP\n"));
    ...
    if (stack == NULL)
    {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "[DbgkSysWin11] 获取当前 IRP 栈失败：返回空指针\n"));
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL,
        "[DbgkSysWin11] IOCTL 开始，代码=0x%08X 名称=%s 输入=%lu 输出=%lu 进程 ID=%lu\n",
    ...
    if (pIrp->AssociatedIrp.SystemBuffer == NULL || cbin != sizeof(USER_DATA))
    {
        ...
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "[DbgkSysWin11] IOCTL 拒绝：缓冲输入无效，地址=%p 大小=%lu，期望至少=%zu；原因：调用方未提供完整的 METHOD_BUFFERED 输入；解决方案：检查 IOCTL 结构体版本和输入缓冲区长度。\n",
        ...
        (requestData->uSize != 0 && requestData->pUserData == 0))
    {
        ntStatus = STATUS_INVALID_PARAMETER;
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "[DbgkSysWin11] IOCTL 审计拒绝：负载长度=%lu、用户指针=%p；允许上限=%lu。原因：长度越界或非零负载没有指针；解决方案：使用匹配版本 DLL，按 USER_DATA 提交有效加密缓冲区。\n",
    ...
    case IOCTL_LOAD_SYMBOLS_TABLE_V2:
    {
        ...
        if (cbout < sizeof(DWORD))
        {
            ...
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
                "[DbgkSysWin11] 符号表 IOCTL 拒绝：输出缓冲区=%lu，小于 DWORD；解决方案：调用方提供至少 4 字节输出缓冲区。\n", cbout));
            break;
        }
...
complete:
    ...
    KdPrintEx((DPFLTR_IHVDRIVER_ID, NT_SUCCESS(ntStatus) ? DPFLTR_INFO_LEVEL : DPFLTR_ERROR_LEVEL,
        "[DbgkSysWin11] IOCTL 完成，代码=0x%08X 名称=%s 状态=0x%08X（%s） 实际返回字节数=%lu\n",
