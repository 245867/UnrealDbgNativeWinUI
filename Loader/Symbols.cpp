...
bool DownloadSymbol_internal(const std::wstring& szModulePath, const std::wstring& path, std::wstring* pdb_path_out, bool Redownload)
{
	...
	if (!File.good())
	{
		Common::ReportSeriousError("%s[%d] 打开文件失败! (错误码：%d)", __func__, __LINE__, GetLastError());
		return false;
	}
	...
	if (!FileSize)
	{
		Common::ReportSeriousError("%s[%d] 获取文件大小失败! (错误码：%d)", __func__, __LINE__, GetLastError());
		return false;
	}
	...
	if (!pRawData)
	{
		...
		Common::ReportSeriousError("%s[%d] 分配内存失败! (错误码：%d)", __func__, __LINE__, GetLastError());
		return false;
	}
	...
	else
	{
		...
		Common::ReportSeriousError("%s[%d] 获取Machine失败! (错误码：%d)", __func__, __LINE__, GetLastError());
		return false;
	}
	...
	if (!pDataDir->Size || IMAGE_DEBUG_TYPE_CODEVIEW != pDebugDir->Type)
	{
		...
		Common::ReportSeriousError("%s[%d] 获取DebugDir失败! (错误码：%d)", __func__, __LINE__, GetLastError());
		return false;
	}
	...
	if (pdb_info->Signature != 0x53445352)
	{
		...
		Common::ReportSeriousError("%s[%d] pdb_info->Signature不匹配! (错误码：%d)", __func__, __LINE__, GetLastError());
		return false;
	}
	...
	if (conversion_ret != 0 || !SizeOut)
	{
		...
		Common::ReportSeriousError("%s[%d] 转换Unicode字符串失败! (错误码：%d)", __func__, __LINE__, GetLastError());
		return false;
	}
	...
	if (Redownload)
	{
		...
		if (!StringFromGUID2(pdb_info->Guid, w_GUID, 100))
		{
			...
			Common::ReportSeriousError("%s[%d] 获取Guid失败! (错误码：%d)", __func__, __LINE__, GetLastError());
			return false;
		}
		...
		if (conversion_ret != 0 || !l_GUID)
		{
			...
			Common::ReportSeriousError("%s[%d] 转换ANSI字符串失败! (错误码：%d)", __func__, __LINE__, GetLastError());
			return false;
		}
		...
		if (Redownload)
		{
			...
			if (hr != S_OK)
			{
				Common::ReportSeriousError("下载符号表失败，系统无法找到指定的资源! "
					"请按照说明书操作使用第三方工具进行下载。(错误码：%d)", GetLastError());
				exit(0);
			}
			//char szBuf[MAX_PATH] = { 0 };
