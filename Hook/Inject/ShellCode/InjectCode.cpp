...
void RemoveHook(DWORD dwPid)
{
	try
	{
		...
		{
			logger.Log("打开进程失败（错误码：%d）",GetLastError());
		}
	}
...
BOOL InjectCode(HANDLE hProcess)
{
	...
	if (hProcess)
	{
		...
		//	WriteProcessMemory(hProcess, remoteAddress, ShellCode, sizeof(ShellCode), NULL);
			...
			//{
			//	error = GetLastError();
			//	outDebug((TCHAR*)_T("启动远程线程失败！(错误码:%d)"), error);
			//}
		...
		//{
		//	error = GetLastError();
		//	outDebug((TCHAR*)_T("分配远进程内存失败！(错误码:%d)"), error);
		//}
		...
		{
			error = GetLastError();
			outDebug((TCHAR*)_T("启动远程线程DbgBreakPoint失败！(错误码:%d)"), error);
		}
	}
	else
	{
		error = GetLastError();
		outDebug((TCHAR*)_T("打开进程句柄失败！(错误码:%d)"), error);
	}
	return boRet;
}
