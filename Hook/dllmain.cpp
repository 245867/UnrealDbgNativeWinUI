	...
	LPVOID lpReserved)   // reserved
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		logger.Log("[INFO] Hook DLL 已附加：模块=%p pid=%lu", hinstDLL, GetCurrentProcessId());
		InitGlobalVariables();
		InitFunction();
		SetupHook();
		break;
	...
	case DLL_PROCESS_DETACH:
	{
		...
		logger.Log("[INFO] Hook DLL 已分离：保留参数=%p", lpReserved);
		UnHook();
		break;
	}
...
void SetupHook()
{
	...
	if (InitializeDevice())
	{
		...
		logger.Log("[INFO] Hook 设备初始化成功，API 钩子安装完成");
	}
	else
	{
		logger.LogError("Hook 初始化驱动设备：InitializeDevice", GetLastError());
	}
	logger.Log("[INFO] SetupHook 结束");
}

void UnHook()
{
	logger.Log("[INFO] UnHook 开始");
...
BOOL InitializeDevice()
{
	...
	{
		logger.LogError("Hook 打开驱动设备：CreateFile", GetLastError());
		return FALSE;
	}
	else
	{
		logger.Log("[INFO] Hook 已连接 \\\\.\\UnrealDbg：句柄=%p", g_hGeneralDriverDevice);
		return TRUE;
	}
}
...
PVOID MyGetProcAddress(HMODULE modBase, TCHAR* modName, LPCSTR lpProcName)
{
	...
	{
		//error = GetLastError();
		//outDebug((TCHAR*)_T("[MyGetProcAddress] 打开文件失败！(错误码:%d)"), error);
		return NULL;
	}
...
int Power()
{
	...
	if (OpenProcessToken(GetCurrentProcess(), TOKEN_ALL_ACCESS, &hToken))
	{
		if (LookupPrivilegeValue(NULL, SE_DEBUG_NAME, &luid))
		{
			...
			{
				error = GetLastError();
				outDebug((TCHAR*)_T("提权失败！(错误码:%d)"), error);
			}
		}
		else
		{
			error = GetLastError();
			outDebug((TCHAR*)_T("LookupPrivilegeValue失败！(错误码:%d)"), error);
		}
	}
	else
	{
		error = GetLastError();
		outDebug((TCHAR*)_T("OpenProcessToken失败！(错误码:%d)"), error);
	}
