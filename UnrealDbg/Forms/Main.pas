{
  说明：RAD Studio 11 编译 64 位程序时，请使用与目标平台匹配的配置，否则可能无法正常调试。
}
...
const
  _STR_KEY = '9dd14d00f5dd71bd';  {用于授权校验的 16 位 MD5 密钥}
  ...
  crScope = 1;  //Delphi 的关键状态标志；由授权流程设置，用于避免重复初始化
...
begin
  if Handle <> 0 then begin
    ...
    if DC <> 0 then begin
      try
        WndWidth := GetSystemMetrics(SM_CXBORDER); //窗口边框宽度，单位为像素
        WndHeight := GetSystemMetrics(SM_CYBORDER); //窗口边框高度，单位为像素

        Rgn := CreateRectRgn(0, 0, 0, 0);
        Pen := CreatePen(PS_INSIDEFRAME, 3 * WndWidth, RGB(0, 0, 0));  //创建边框画笔
        original_pen := SelectObject(DC, Pen);
        original_brush := SelectObject(DC, GetStockObject(NULL_BRUSH)); //选择透明画刷
        SetROP2(DC, R2_NOT);
        if GetWindowRgn(Handle,Rgn) <> 0 then begin
          SysColor := GetSysColor(COLOR_WINDOWFRAME);
          Brush := CreateHatchBrush(HS_DIAGCROSS, SysColor); //45 度对角线阴影
          FrameRgn(DC, Rgn, Brush, 3 * WndWidth, 3 * WndHeight);
          DeleteObject(Brush);
        end else begin
          frameWidth := GetSystemMetrics(SM_CXFRAME);        // SM_CXFRAME 边框宽度，单位为像素
          frameHeight := GetSystemMetrics(SM_CYFRAME);       // SM_CYFRAME 边框高度，单位为像素
          screen_Width := GetSystemMetrics(SM_CXSCREEN);       // SM_CXSCREEN 主显示器屏幕宽度
          screen_height := GetSystemMetrics(SM_CYSCREEN);      // SM_CYSCREEN 主显示器屏幕高度
          ...
          if IsZoomed(Handle) then begin
            ...
            Rectangle(DC, 0, 0, Rect.right - Rect.left, Rect.bottom - Rect.top);//绘制矩形边框
          end;
        end;
    ...
    if Button = mbLeft then begin
      ...
      if m_SelectedProcessId <> 0 then begin
        if g_boEnabled_tl_confrontation_TL then begin
          ...
          btn := MessageBoxW(0,PChar(sText),'选择目标进程:', MB_YESNO or MB_SYSTEMMODAL);
    ...
    if g_boEnabled_tl_confrontation_TL then begin
      if g_boHandlerGetTickCountCheck_TL then begin
        ...
        base := GetProcessModuleBase(m_targetProcessId,'TL.exe');
          Handler_TLDetection(base); //处理 TL 检测
      end;
...
begin
  try
    sLog.outInfo('=== 加载版权信息 ===');
    m_Caption := '默认版权   by: Bug提供   QQ群:740336586';
    m_Log1 := '默认版权 QQ群:740336586';

    sPath := ExtractFilePath(Application.ExeName);
    sLog.outDebug('应用目录: ' + sPath);
    if sPath <> '' then begin
      sFileName := sPath + _STR_COPYRIGHT;
      sLog.outDebug('版权文件路径: ' + sFileName);
      if FileExists(sFileName) then begin
        sLog.outDebug('版权文件存在，开始解密');
        decryptedDataLen := D_encryption_DecryptDataFromFile(sFileName,_STR_KEY,nil);
        if decryptedDataLen > 0 then begin
          sLog.outInfo('版权文件解密成功，长度: ' + IntToStr(decryptedDataLen));
          ...
          sLog.outDebug('JSON数据: ' + jsonStr);
          json := json.ParseJSONValue(jsonStr) as TJSONObject;
          if Assigned(json) then begin
            m_Caption := json.Values['版权来源'].AsType<string>;
            m_Log1 := json.Values['QQ群日志'].AsType<string>;
            sLog.outInfo('版权标题: ' + m_Caption);
            sLog.outInfo('日志前缀: ' + m_Log1);
            json.Free;
          end else begin
            sLog.outError('JSON解析失败');
          end;
          SetLength(PlainText,0);
        end else begin
          sLog.outError('版权文件解密失败，解密长度: ' + IntToStr(decryptedDataLen));
        end;
      end else begin
        sLog.outDebug('版权文件不存在，使用默认值');
      end;
    end else begin
      sLog.outError('无法获取应用目录');
    end;
  except on e:Exception do begin
    sLog.outError('加载版权信息失败: ' + e.Message);
  end;
  sLog.outInfo('=== 版权信息加载结束 ===');
end;
...
begin
  ...
  // 添加详细日志开始
  sLog.outInfo('=== UnrealDbg 启动日志开始 ===');
  sLog.outDebug('开始初始化系统');

  LoadCopyright;
  Caption := m_Caption;

  sLog.outDebug('加载版权信息完成');
  sLog.outDebug('界面标题设置完成');

  sLog.outDebug('开始获取系统信息');
  GetWindowsNTVer;
  sLog.outDebug('系统信息获取完成');

  Screen.Cursors[crScope] := LoadCursor(HInstance,'Cursor_1');
  sLog.outDebug('鼠标光标加载完成');

  g_boStartService := False;
  g_boLoginSuccess := False;
  sLog.outDebug('全局变量初始化完成');

  VMProtectBeginVirtualization('VMP');
  g_Authentication := TAuthentication.Create;
  sLog.outDebug('认证对象创建完成');

  if g_Authentication.cdkeyLogin then begin
    sLog.outDebug('授权检查通过，开始加载调试器列表');
    //加载版权信息
    SetPrivilege;
    g_DebuggerList := TGList.Create;
    g_EventHandlerThread := TEventHandlerThread.Create(False);
    LoadDebuggerList;
    sLog.outDebug('调试器列表加载完成');
    LoadCheckBoxState;
    sLog.outDebug('勾选状态加载完成');
  end else begin
    sLog.outError('授权检查失败！程序将退出');
    MessageBoxW(0,'授权检查失败!','错误:',MB_ICONERROR);
    ExitProcess(0);
  end;
  VMProtectEnd;
  sLog.outInfo('=== UnrealDbg 启动完成 ===');
end;
...
var
  I: Integer;
begin
  sLog.outInfo('=== UnrealDbg 关闭日志 ===');
  sLog.outDebug('开始释放资源');

  if g_boLoginSuccess then begin
    if g_LockThread <> nil then begin
      sLog.outDebug('停止并释放LockThread');
      g_LockThread.Stop;
      g_LockThread.WaitFor;
      g_LockThread.Free;
    end;
    ...
    if g_EventHandlerThread <> nil then begin
      sLog.outDebug('停止并释放EventHandlerThread');
  ...
  sLog.outDebug('认证对象释放完成');
  sLog.outInfo('=== UnrealDbg 完全关闭 ===');
end;
  ...
  try
    if Assigned(DbgListView.Selected) then begin
      ...
      if Assigned(item) then begin
        ...
        filePath := item.SubItems.Strings[1];
        if (fileName <> '') and (filePath <> '') then begin
          if Application.MessageBox('是否确定移除选中的项？', '确认信息', MB_YESNO + MB_ICONQUESTION) = IDYES then begin
            g_DebuggerList.Lock;
            try
...
begin
  try
    ...
    OpenDialog1.Execute;
    filename := ExtractFileName(OpenDialog1.FileName);  //获取文件名
    filePath := OpenDialog1.FileName;  //获取文件路径
    ...
    if (filename <> '') and (filePath <> '') then begin
      if not DebuggerItemExists(filename,filePath) then begin
        ...
        Application.MessageBox('该调试器已经在列表中..', '警告:', MB_ICONWARNING);
      end;
    end;
  except on e:Exception do
